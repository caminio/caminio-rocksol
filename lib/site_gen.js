...
module.exports = function SiteGen( caminio ){
  ...
  generator.compilePage = function compilePage( res, webpage, done ){
    ...
    function searchSiblings( arr, next ){
      globalOp.ancestors = arr;
      genMethods.getChildrenOfWebpage( globalOp.ancestors[0], next);
    }
    ...
    function generateSite( err, arr ){
      ...
      function generateLocaleContent( translation, next ){
        var options = {
          ancestors: globalOp.ancestors,
          siblings: globalOp.siblings,
          children: globalOp.children,
          webpage: webpage
        };

        console.log('called with: ', translation.content );
        options.markdownContent = markdown.toHTML( translation.content );

        options.translation = {
          locale: translation.locale
        };
        console.log(options.translation.locale,'MADE WITH', options.markdownContent, 'END' );
    ...
    function createFile( content, translation, langs ){  
      ...
      var fileName = join(  
        getFolder( publicPath, globalOp.ancestors ), 
        inflection.underscore( webpage.name ) + ending 
      );
      replaceFilePattern( content );
      console.log('THE FILENAME:', fileName, 'THE CONTENT: ', content );
