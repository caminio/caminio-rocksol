...
module.exports = function SiteGen( caminio ){
   ...
   *  @param done
   */
  generator.compilePage = function compilePage( res, webpage, params, layout, done ){
    var domain = res.locals.currentDomain;

    var siteFileManager = require('./site_file_manager')( caminio, domain.getContentPath(), layout );

    var info = {};

    var err = siteFileManager.checkFolders( layout );
    if( err )
      done( err );
    ...
    //   if( !fs.existsSync( siteFileManager.layoutFile ) )
    //     return done('layout '+siteFileManager.layoutFile+' was not found');
    ...
    function generateLocaleContent( translation, cbAfterCompile ){
       ...
       *
       */
      function runJSFiles( next ){
        //TODO
        // var layoutJSFile = ....
        // var applicationJSFile = .....
        WebpageMethods.runJS( webpage, siteFileManager.jsFile, options, function(){
          //WebpageMethods.runJS( webpage, layoutJSFile, options, next );
          //WebpageMethods.runJS( webpage, siteFileManager.jsFile, options, next );
          next();
        });
      }
      ...
      function runJadeCompiler( next ){
        options.translation.content = WebpageMethods.compileJade( siteFileManager.layoutFile, options );
        next();
      }
      ...
      function finish( err, content ){
        ...
        if( webpage.translations.length > 1 )
          siteFileManager.createFile( content, translation.locale, fileParams );
        else
          siteFileManager.createFile( content, null, fileParams );
    ...
    function noIndexFile( translation ){
      if( layout === 'index' && ( translation.locale  === res.locals.currentDomain.lang ) )
          return true;
      return false;
    }    
