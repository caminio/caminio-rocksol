 ...
 * @Last Modified time: 2014-05-02 00:31:00
 *
...
module.exports = function( caminio ){
  ...
  function runParsers( content, options, done ){
    console.log('REACHED END OF COMPILING!!!!!!');
    // FOR FUTURE PARSERS
    done( null, content, options );    
  }
  ...
  function addLinkHelper( attrs ){
    ...
    var currentPath = fileManager.getAncestorPath( attr.locals.ancestors );

    return function( name, namespace, extension ){
      var path;
      extension = extension || '.htm';
      if( namespace )
        path = settings.domainPath;
      else
        path = currentPath;

      var file = join( path, namespace, name, locale, extension );

      if ( fs.existsSync( file ) )
        return path;
      else
        return join( path, namespace, name, extension );
    };
