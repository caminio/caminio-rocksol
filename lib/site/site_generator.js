 ...
 * @Last Modified time: 2014-06-02 19:44:32
 *
...
module.exports = function( caminio ){
  ...
  var dependencies = require('./site_dependencies')( caminio );
  var settings = {};
  var gCompileContent;
   ...
   *         config/site.js file the settings are read and written into the the local
   ...
   *  @param options.indexActivated If a object is compiled which has children,
   *         the file will be named 'index' and is written into a folder named
   *         after the filename of the object.
  ...
  function compileTranslations( object, options, callback ){
    ...
    async.eachSeries( object.translations, function( translation, nextTranslation ){
      ...
      gCompileContent( translation.content, options, function( err, compiledContent ){
        ...
        if( settings.upload )
          fileManager.fileUpload( options, compiledContent, nextTranslation );
        else
  ...
  function output( options, compiledContent, paths, next ){
    ...
    if( currentDomain.isCaminioHosted ){ 
      ...
      fileManager.fileUpload( options, compiledContent, next );
  ...
  function fileOutput( options, compiledContent, object, next ){
    ...
    if( options.isPublished ){
      ...
      if( options.locals.children && options.locals.children.length > 0 && options.indexActivated )
         paths.push( fileManager.getIndexPath( object, options.locals.ancestors ) );
      else
