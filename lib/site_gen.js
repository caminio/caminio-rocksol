...
module.exports = function SiteGen( caminio ){
  ...
  generator.compilePage = function compilePage( res, webpage, params, done ){
    ...
    function generateSite( err, children ){
      ...
      WebpageMethods.setUrlPath( [ webpage ], info.ancestors );
      WebpageMethods.setUrlPath( info.siblings, info.ancestors );
    ...
    function compileAll(){
      console.log('WE ARE HERE?', params, webpage.name);
      ...
      var options = {
        ...
        t: function( str ){
          ...
          if( fs.existsSync( translationFile +'.js') ){
            var translationStr = require( translationFile )[ str ];
    ...
    function createFile( content, locale ){  
      ...
      var fileName = join(  
        ...
        WebpageMethods.underscore( webpage.name ) + ending 
      );
