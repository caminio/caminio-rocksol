 ...
 * @Last Modified time: 2014-04-12 19:47:59
 *
...
module.exports = function( caminio ) {
  ...
  function runSearch( data, done ){
    ...
    if( !first ){
        ...
        if( snippet.options.array && attributes[ snippet.options.array ] ){
          var pebbleFnArray  = attributes[ snippet.options.array ].map( function( pebble, index ){
            return function( nextSerie ){
              attributes.pebble = pebble;
              attributes.pebble.index = index;
              webpageMethods.runJS( data, snippet.path + '/' + snippet.name +".js", attributes, function(){
                localContent += webpageMethods.compileJade( snippet.path + "/" + snippet.name +".jade", attributes );
                nextSerie();
              });
            }
          });
    ...
    function checkForPebbles( next ){
      if( pebbles && pebbles.length ){
        ...
        list.forEach( function( snippet ){
          if( snippet.type === 'pebble')
            pebbleList.push( snippet );
        });
        processPebbles( pebbleList, next ); 
      }
      else 
        next();       
    }
    ...
    function checkForRubbles( next ){
      if( rubbles && rubbles.length ){
        var rubbleList = [];
        list.forEach( function( snippet ){
          if( snippet.type === 'rubble')
            rubbleList.push( snippet );
        });
        processRubbles( rubbleList, next );
      }
      else
        next();
    }
    ...
    function addToList( original ){
      ...
      if( name.match( pebbleRegexp ) ){
        addValues( pebbles, 'pebble' );
      } else if( name.match( rubbleRegexp ) ){
        addValues( rubbles, 'rubble' );
      } else {
   ...
   *  @param {[String]} list of pebbles
   *  @param cb
   */
  function processPebbles( pebbleList, cb ){
    var webpageSearch = [];
    var globalSearch = [];
    ...
    function getSearchParams( next ){
      pebbleList.forEach( function( element ){
        if( element.options && !element.options.global )
          webpageSearch.push( element.name );
        else if( element.options && element.options.global === 'true' )
    ...
    function run( err, pebbles ){
      ...
      function recursiv( pebble, next ){
        pebble.meta = webpageMethods.getElementFromArray( pebbleList, 'name', pebble.name );
  ...
  function processRubbles( rubbleList, cb ){
    async.forEach( rubbleList, recursiv, cb );
