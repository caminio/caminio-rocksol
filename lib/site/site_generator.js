 ...
 * @Last Modified time: 2014-05-27 18:03:29
 *
...
module.exports = function( caminio ){
  ...
  function gCompileLayout( layout, options, callback ){
    ...
    gCompileContent( '', options, function( err, compiledContent ){
      if( settings.upload )
        fileUpload( options, compiledContent, callback );
      else{
  ...
  function fileUpload( options, compiledContent, next ){
    ...
    try{
      ...
      var destination = domain.isCaminioHosted ? 'file://' + options.contentPath + '/public/' : domain.remoteAddr;
      if( typeof settings.upload === 'string' )
        destination = destination  + settings.upload + '/';
