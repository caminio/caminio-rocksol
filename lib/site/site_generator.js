 ...
 * @Last Modified time: 2014-04-28 16:11:47
 *
 ...
 */
function SiteGenerator( caminio, path, layouts ){
  var PeRuProcessor = require('./pe_ru_bble/pe_ru_bble_processor');
  var Deps = require('./site/site_dependencies');
  var SiteFileManager = require('./../../lib/site/site_file_manager');
  
  fileManager = new SiteFileManager();

  // TODO remove methods
  WebpageMethods = require('./webpage_methods')( caminio );

  settings = readSettingsFile( path );
  settings.contentPath = path;
  settings.layoutsPath = layouts ? layouts : 'layouts';
...
function compileTranslations( object, options, callback ){
  ...
  var siteFileManager = require('./site_file_manager')( c, settings.contentPath, object.layout );
...
function getJsFiles( files, settings, layout ){
  ...
  var layoutsPath = join( settings.contentPath, settings.layoutsPath );
  var jsFiles = {
    layoutFile: join( layoutsPath, layout, layout + '.js' ),
    applicationFile: join( layoutsPath, 'application.js' ),
    domainFile: join( settings.contentPath, 'domain.js' )
  };
...
function runJsFiles( content, options, done ){  
  ...
  async.each( options.jsFiles, function( jsFile, next){
    WebpageMethods.runJS( options.currentObject, jsFile, options.locals, next );
  }, function(){
...
function runPebbleRubbleProcessor( content, options, done ){
  options.contentPath = settings.contentPath;
