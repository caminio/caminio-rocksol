 ...
 * @Last Modified time: 2014-04-29 16:38:03
 *
...
function gCompileArray( array, options, callback ){
  ...
  async.each( array, function( object, done ){
    console.log('compiling ', object.name );
...
function compileTranslations( object, options, callback ){
  ...
  fileManager.cleanDirectory( fileManager.getAncestorPath( options.locals.ancestors, object.name ) );
  ...
  async.eachSeries( object.translations, function( translation, nextTranslation ){
    ...
    [ 'webpage', 'ancestors', 'children', 'webpage' ].forEach( function( key ){
      setTrans( options.locals[ key ], options.locale );
    });
    ...
    gCompileContent( translation.content, options, function( err, compiledContent ){
      ...
      if( options.isPublished )
        paths.push( fileManager.getObjectPath( options.locals.webpage, options.locals.ancestors ) );

      if( object.layout === 'index' && object.name === 'index' )
