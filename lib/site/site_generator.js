 ...
 * @Last Modified time: 2014-05-02 01:34:08
 *
...
module.exports = function( caminio ){
  ...
  function compileTranslations( object, options, callback ){
    ...
    async.eachSeries( object.translations, function( translation, nextTranslation ){
      ...
      [ 'doc', 'webpage', 'ancestors', 'children', 'siblings' ].forEach( function( key ){
        methods.setCurTranslation( options.locals[ key ], options.locale );
        console.log('SET CUR TRANS: ', options.locals[key]);
      });
  ...
  function runLayoutCompiler( content, options, done ){
    ...
    console.log('exists: ',attributes.linkHelper);
  ...
  function addLinkHelper( attrs ){
    ...
    return function( name, namespace, extension ){
      var path = '/';
      ...
      var file = join( path, name + '.' + locale + extension );

      if ( fs.existsSync( join( attrs.currentDomain.getContentPath(), 'public', file ) ) )
        return file;

      var file = join( path, name + extension );
      if ( fs.existsSync( join( attrs.currentDomain.getContentPath(), 'public', file ) ) )
        return file;
